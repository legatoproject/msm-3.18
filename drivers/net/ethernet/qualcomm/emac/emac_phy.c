...
/* SWISTART */
	...
	retval = phy_write(adpt, MII_MMD_CTRL, 0x7);
	if (retval)
		return retval;
	retval = phy_write(adpt, MII_MMD_DATA, 0x8016);
	if (retval)
		return retval;
	retval = phy_write(adpt, MII_MMD_CTRL, 0x4007);
	if (retval)
		return retval;
	retval = phy_write(adpt, MII_MMD_DATA, &value_MMD7_CLK_25M);
	if (retval)
		return retval;
	retval = phy_write(adpt, MII_MMD_DATA, value_MMD7_CLK_25M | 0x1000);
	if (retval)
		return retval;
	retval = phy_write(adpt, MII_MMD_DATA, &value_MMD7_CLK_25M);
	if (retval)
