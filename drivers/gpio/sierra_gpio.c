...
static ssize_t alias_gpio_num_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	desc = gpio_to_desc(ext->gpio_num);
	if (desc && desc->chip)
		return sprintf(buf, "%d,%d\n", desc->chip->base,
				ext->gpio_num - desc->chip->base);
	else
...
static /* const */ DEVICE_ATTR(unexport, 0200, NULL, unexport_store);
 ...
 */
static ssize_t alias_map_define(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t len)
{
	int			status;
	unsigned int		base = 0;
		...
		if (gpio_alias_map[i].gpio_name) {
			...
			if (desc && desc->chip)
				status += snprintf(buf + status, PAGE_SIZE - status, "%4d,%-4d %c \"%s\"\n",
							desc->chip->base,
							gpio_alias_map[i].gpio_num - desc->chip->base,
 ...
 * Returns the number of characters to display
 */
static ssize_t gpiochip1_show_mask(struct device *dev, struct device_attribute *attr, char *buf)
{
	const struct gpio_chip  *chip = gpio_to_desc(1)->chip;

	return sprintf(buf, "0x%08x%08x\n0x%08x%08x\n",
			(u32)(chip->mask[0] >> 32), (u32)chip->mask[0],
			(u32)(chip->mask[1] >> 32), (u32)chip->mask[1]);
}
 ...
 * Returns the number of characters to display
 */
static ssize_t gpiochip1_show_mask_v2(struct device *dev, struct device_attribute *attr, char *buf)
{
	const struct gpio_chip	*chip = gpio_to_desc(1)->chip;
	int			len = 0;
	int			i;
...
static int sierra_gpio_probe(struct platform_device *pdev)
{
	...
	for_each_property_of_node(np, pp) {
		...
		if (gpio >= 0) {
			...
			if (desc && desc->chip && (desc->chip->bitmask_valid)
				&& (desc->bit_in_mask >= 0) && (desc->bit_in_mask < desc->chip->ngpio)) {
				int nmask = desc->bit_in_mask / (sizeof(desc->chip->mask[0]) * 8);
				int nbit = desc->bit_in_mask % (sizeof(desc->chip->mask[0]) * 8);
				desc->owned_by_app_proc = (desc->chip->mask[nmask] >> nbit) & 1ULL;
				if (desc->chip->max_bit < desc->bit_in_mask)
					desc->chip->max_bit = desc->bit_in_mask;
			}
