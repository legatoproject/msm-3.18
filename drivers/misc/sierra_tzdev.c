              ...
              uint8_t* data,uint32_t dlen,uint8_t* signeddata, uint32_t* signeddata_len)
{
  ...
  {
    pr_err("%s(): GEN KEY: Memory allocation for rsa_sign_data_test_req failed\n", __func__);
    goto end;
  }
  ...
  uint8_t *modulus,uint32_t *modulus_size,uint8_t *public_exponent,uint32_t *public_exponent_size)
{
  ...
  {
    pr_err("%s(): GEN KEY: Memory allocation for rsa_export_key_test_req/resp failed\n", __func__);
    return -1;
  }
  ...
  tz_rsa_digest_pad_algo_t padding, crypto_rsa_key_blob_type* key_blob_ptr)
{
  ...
  {
    pr_err("%s(): Memory allocation for rsa import keys req/resp commands failed\n", __func__);
    return -1;
  }
...
static long sierra_tzdev_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
  ...
  ret = copy_from_user(&scm_buf, scm_buf_user_ptr, sizeof(struct tzdev_op_req));
  if (ret)
      return -EFAULT;
  ...
  plain_data_len = scm_buf.plain_dlen;
  sealed_data_len = scm_buf.encrypted_len;
  key_material_len = scm_buf.encklen;
  ...
  switch (cmd) 
  {
    ...
    case TZDEV_IOCTL_KEYGEN_REQ:
      {
        ...
        if ((output_len > key_material_len) ||
            copy_to_user(scm_buf.enckey, (void *)key_material, output_len) ||
    ...
    case TZDEV_IOCTL_SEAL_REQ:
      {
        if (copy_from_user((void *)plain_datap, scm_buf.plain_data, plain_data_len) ||
            copy_from_user((void *)key_material, scm_buf.enckey, key_material_len))
        {
        ...
        if ((output_len > sealed_data_len) ||
            ...
            copy_to_user(scm_buf.encrypted_buffer, (void *)sealed_datap, output_len))
        {
    ...
    case TZDEV_IOCTL_UNSEAL_REQ:
      {
        if (copy_from_user((void *)sealed_datap, scm_buf.encrypted_buffer, sealed_data_len) ||
            copy_from_user((void *)key_material, scm_buf.enckey, key_material_len))
        {
          printk(KERN_ERR "%s_line%d: copy_from_user failed\n", __func__,__LINE__);
          rc = -EFAULT;
        ...
        if ((output_len > plain_data_len) ||
            ...
            copy_to_user(scm_buf.plain_data, (void *)plain_datap, output_len))
        {
          printk(KERN_ERR "%s_line%d: copy_to_user failed\n", __func__,__LINE__);
          rc = -EFAULT;
          break;
      ...
      case TZDEV_IOCTL_RSA_KEYPAIR_REQ:
        {
          ...
          if (key_blob.key_material_len > key_material_len ||
              copy_to_user(scm_buf.enckey, (void *)key_material, key_blob.key_material_len) ||
      ...
      case TZDEV_IOCTL_RSA_VERIFY_REQ:
        {
          ...
          key_blob.key_material = (crypto_rsa_key_type* )key_material;
          if (copy_from_user((void *)plain_datap, scm_buf.plain_data, plain_data_len)||
              copy_from_user((void *)sealed_datap, scm_buf.encrypted_buffer, sealed_data_len) ||
              copy_from_user((void *)key_material, scm_buf.enckey, key_material_len))
          {
            printk(KERN_ERR "%s_line%d: copy_from_user failed\n", __func__,__LINE__);
            rc = -EFAULT;
      ...
      case TZDEV_IOCTL_RSA_EXPORTKEY_REQ:
        {
          ...
          if (copy_from_user((void *)key_material, scm_buf.enckey, key_material_len))
          {
            printk(KERN_ERR "%s_line%d: copy_from_user failed\n", __func__,__LINE__);
            rc = -EFAULT;
            break;
          ...
          if (copy_to_user(&(scm_buf_user_ptr->plain_dlen), &plain_data_len, sizeof(uint32_t)) ||
              copy_to_user(scm_buf.plain_data, (void *)plain_datap, plain_data_len) ||
              copy_to_user(&(scm_buf_user_ptr->encrypted_len), &sealed_data_len, sizeof(uint32_t)) ||
              copy_to_user(scm_buf.encrypted_buffer, (void *)sealed_datap, sealed_data_len))
          {
                  printk(KERN_ERR "%s_%d: copy_to_user failed\n", __func__,__LINE__);
                  rc = -EFAULT;
                  break;
