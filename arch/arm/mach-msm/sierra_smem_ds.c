 ...
 *
 * Parms:   [out]ds_smem_bufp dual system infomation buffer pointer
 *
 * Return:   None
 *
 * Abort:    None
...
void sierra_ds_smem_get(struct ds_smem_message_s * ds_infop)
{
	...
	if (NULL == ds_smem_bufp)
	{
		ds_infop->ssid_modem_idx = DS_SSID_SUB_SYSTEM_1;
		ds_infop->ssid_lk_idx = DS_SSID_SUB_SYSTEM_1;
		ds_infop->ssid_linux_idx = DS_SSID_SUB_SYSTEM_1;
		ds_infop->is_changed = 0;
		ds_infop->bad_image = DS_IMAGE_CLEAR_FLAG;
...
EXPORT_SYMBOL(sierra_ds_smem_get);
...
void sierra_ds_smem_erestore_info_get(struct ds_smem_erestore_info *efs_restore_infop)
{
	...
	if (NULL != virtual_addr)
	{
		...
		if ((DS_MAGIC_EFSB == efs_restore->magic_beg) &&
			(DS_MAGIC_EFSE == efs_restore->magic_end) &&
			(crc32_le(~0, (void *)efs_restore, DS_ERESTORE_CRC_SZ)))
		{
...
EXPORT_SYMBOL(sierra_ds_smem_erestore_info_get);
