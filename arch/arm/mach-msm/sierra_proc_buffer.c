...
char global_buffer[SIERRA_PROC_STRING_MAX_LEN+1] = {0};
...
static int __init proc_dual_system_buffer_init(void) {
	proc_file = proc_create("proc_buffer", S_IRUGO, NULL, &sierra_proc_buf_fops);

	return 0;
}

static void __exit proc_dual_system_buffer_exit(void) {
...
MODULE_LICENSE("GPL");
module_init(proc_dual_system_buffer_init);
module_exit(proc_dual_system_buffer_exit);
